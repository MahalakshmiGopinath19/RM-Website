  ...
  return (
    <>
      <header className={`fixed top-0 w-full z-50 transition-all duration-300 ${scrolled ? 'bg-white/95 backdrop-blur-md shadow-lg' : 'bg-white shadow-md'}`}>
        <div className="bg-gradient-to-r from-[#B71C1C] to-[#D32F2F] text-white text-center py-2.5 text-sm font-medium tracking-wide">
        ...
        <nav className="container mx-auto px-4 lg:px-6 py-3 flex justify-between items-center">
          ...
          <div className="hidden lg:flex items-center space-x-6 xl:space-x-8">
            {['Home', 'Services', 'Products', 'Career', 'About', 'Blog', 'Contact'].map((item) => (
              <Link key={item} href={item === 'Home' ? '/' : `/${item.toLowerCase()}`} className="relative text-[#D32F2F] font-medium group text-sm xl:text-base">
                {item}
                <span className="absolute -bottom-1 left-0 w-0 h-0.5 bg-[#D32F2F] transition-all duration-300 group-hover:w-full"></span>
              </Link>
            ))}
          </div>
          ...
          <div className="hidden lg:flex items-center space-x-4 xl:space-x-5">
            <div className="flex items-center space-x-3 border-r border-gray-200 pr-4 xl:pr-5 text-gray-500">
              <a href="https://www.facebook.com/p/Rainbow-Media-100091745963846/" target="_blank" className="hover:text-[#D32F2F] transition transform hover:scale-110"><FaFacebookF /></a>
              <a href="https://www.instagram.com/rainbowmedia_digital/" target="_blank" className="hover:text-[#D32F2F] transition transform hover:scale-110"><FaInstagram /></a>
              <a href="#" target="_blank" className="hover:text-[#D32F2F] transition transform hover:scale-110"><FaYoutube /></a>
              <a href="https://www.linkedin.com/company/rainbow-media/" target="_blank" className="hover:text-[#D32F2F] transition transform hover:scale-110"><FaLinkedinIn /></a>
              <a href="https://x.com/rmedia_digital" target="_blank" className="hover:text-[#D32F2F] transition transform hover:scale-110"><FaTwitter /></a>
            </div>
            <button onClick={() => setShowModal(true)} className="bg-gradient-to-r from-[#D32F2F] to-[#B71C1C] text-white px-5 py-2 rounded-full font-semibold text-sm shadow-md hover:shadow-xl transition-all duration-300 hover:-translate-y-0.5">
              Get started
            </button>
          </div>

          <button onClick={() => setMobileMenuOpen(!mobileMenuOpen)} className="lg:hidden text-2xl text-[#D32F2F] focus:outline-none">
            {mobileMenuOpen ? '✕' : '☰'}
          </button>
        </nav>
          ...
          <div className="lg:hidden absolute top-full left-0 w-full bg-white shadow-xl border-t z-40 p-5 animate-slideDown">
            <div className="flex flex-col space-y-3">
              {['Home', 'Services', 'Products', 'Career', 'About', 'Blog', 'Contact'].map(i => (
                <Link key={i} href={i === 'Home' ? '/' : `/${i.toLowerCase()}`} onClick={() => setMobileMenuOpen(false)}>{i}</Link>
              ))}
              <button onClick={() => { setShowModal(true); setMobileMenuOpen(false); }} className="mt-2 bg-gradient-to-r from-[#D32F2F] to-[#B71C1C] text-white py-2.5 rounded-full font-semibold">
                Get started
              </button>
            </div>
          </div>
...
<div className="bg-[#B71C1C] text-white p-10 md:p-12 w-full md:w-[48%] relative flex flex-col justify-center overflow-hidden">
  ...
  {/* Diagonal slash fix: added pointer-events-none so it doesn't block text */}
  ...
  {/* Content container: z-10 ensures it stays above the slash */}
  <div className="relative z-10 flex flex-col h-full justify-center">
    <div className="bg-white/20 text-white px-3 py-1 rounded text-xs font-bold uppercase tracking-widest inline-block mb-6 self-start border border-white/20">Why Choose us</div>
...
<select 
  ...
  className="w-full border-b border-gray-300 py-3 focus:outline-none focus:border-[#B71C1C] text-gray-900 bg-transparent text-sm font-medium"
>
...
<input 
  ...
  placeholder="Name" 
  onChange={handleChange} 
  className="w-full border-b border-gray-300 py-3 focus:outline-none focus:border-[#B71C1C] text-gray-900 placeholder:text-gray-400 text-sm" 
  required 
/>
...
<textarea 
  ...
  onChange={handleChange} 
  className="w-full border-b border-gray-300 py-3 focus:outline-none focus:border-[#B71C1C] text-gray-900 placeholder:text-gray-400 text-sm resize-none" 
  rows={1} 
/>
