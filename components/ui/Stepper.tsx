...
function StepIndicator({ step, currentStep, onClickStep, disableStepIndicators }: {
  ...
  return (
    <motion.div onClick={handleClick} className="step-indicator" style={disableStepIndicators ? { pointerEvents: 'none', opacity: 0.5 } : {}} animate={status} initial={false}>
      <motion.div
        variants={{
          inactive: { scale: 1, backgroundColor: '#f0ece8', color: '#bbb' },
          active: { scale: 1, backgroundColor: '#E0A36A', color: '#E0A36A' },
          complete: { scale: 1, backgroundColor: '#E0A36A', color: '#fff' }
        }}
      ...
      >
        {status === 'active' ? (
          ...
          <span className="step-number" style={status === 'complete' ? { color: '#fff' } : {}}>{step}</span>
        )}
      </motion.div>
    </motion.div>
  );
